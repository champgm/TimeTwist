This is an android wear app that controls 3 timers. I want to add a new feature.
...
There are 2 line segments separating the 3 timers. These are not named explicitly in the code, but I am going to give them names here for clarity.
* segment_0_2 separates timer0 from timer2, it is drawn from the center of the watch face to the right edge.
...
* transition_button_0_2 should be placed such that its center is at the midpoint of segment_0_2
* transition_button_1_2 should be placed such that its center is at the midpoint of segment_1_2

These new buttons should have 4 states each. Each tap of a button should transition that button to the next state.
* transition_button_0_2
  * default_0_2: a circle, bisected horizontally by a line that exactly matches the color & thickness of segment_0_2
  * transition_0_2 appearance: the same circle described by `default appearance` but instead of being bisected horizontally, it is bisected vertically and, most importantly, that bisecting line has an arrow head pointing DOWNWARDS. This indicates that when timer0 finishes, timer2 should begin executing.
  * transition_2_0 appearance: the same circle described by `default appearance` but instead of being bisected horizontally, it is bisected vertically and, most importantly, that bisecting line has an arrow head pointing UPWARDS. This indicates that when timer2 finishes, timer0 should begin executing.
  * transition_0_2_repeat appearance: the same circle described by `default appearance` but instead of being bisected horizontally, it is bisected vertically and, most importantly, that bisecting line has one arrow head pointing UPWARDS and one arrow head pointing DOWNWARDS. This indicates that
    * when timer0 finishes, timer2 should begin executing
    * when timer2 finishes, timer0 should begin executing
...
Coordinating the logic for these transitions may be tricky and should be done very carefully. Here are the dangers I can think of, but there will be more lurking.
  ...
  * on transition, the completion "big" vibration/sound might get masked by the next timer's "small" startup alert. Suppressing the chained startup alert preserves the important "this timer ended" signal.
